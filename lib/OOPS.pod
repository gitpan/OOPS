...
The following RDBMSs are supported by the code:
...
Once that's fixed, there is a still a problem that it detects some 
deadlocks very slowly: L<http://bugs.mysql.com/bug.php?id=20857>.
...
As OOPS is developed, the schema that OOPS uses changes.  
...
L<OOPS::Fsck> to fix such problems.
...
disk space will cause the tests to fail.  On DragonflyBSD (FreeBSD 4.9)
...
The following applications uses OOPS:
