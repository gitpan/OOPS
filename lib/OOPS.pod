...
(keys and values), a table of large attributes that big the normal column
...
OOPS 1.004 has been tested with PostgreSQL version 8.1.4 on Linux.  
OOPS 1.003 was tested with 7.4.2 and 7.3.5 on Linux and 7.3.5 on
...
OOPS 1.004 has been tested with mysql 4.1.15 on Linux.
OOPS 1.003 was tested with mysql 4.0.16 and 4.0.18 using InnoDB tables. 

The OOPS schema for mysql does not support large (>256 byte) hash
keys.  

Biggest issue: Mysql 4.1.15 (at least) detects some deadlocks very slowly.
L<http://bugs.mysql.com/bug.php?id=20857>.

=item SQLite

DBD::SQLite versions 0.x use SQLite 2.x.  DBD::SQLite versions 1.x use
SQLite 3.x.  DBD::SQLite versions 1.x leak file descriptors and will 
eventually die if you create use and destroy too many OOPS instances.
This prefents DBD::SQLite version 1.x from passing the OOPS regression tests.

If you're using OOPS 1.003 and DBD::SQLite 0.x then upgrade OOPS to 1.004 and
switch to DBD::SQLite2.  Make both changes at the same time.

OOPS 1.004 has been tested with DBD::SQLite2 0.33.
OOPS 1.003 was tested with DBD::SQLite 0.31.

DBD::SQLite2 is not 8-bit clean with respect to C<\0>.  OOPS uses a DBD::SQLite2
feature to translate binary nulls.  A side-effect is that backslash
will be doubled C<\> -> C<\\> in DBD::SQLite2 (the extras will be removed
transparently by DBD::SQLite2.  OOPS does not use the C<counters> table
with DBD::SQLite2 (or DBD::SQLite).  As of 1.004, OOPS doesn't make use of the 
newer (version 1.x) DBD::SQLite features for handling nulls.

L<http://www.hwaci.com/sw/sqlite/>

Biggest issues: SQLite3 fails (L<http://rt.cpan.org/Ticket/Display.html?id=20286>); 
SQLite3 leaks memory (L<http://rt.cpan.org/Ticket/Display.html?id=20288>); 
SQLite2 and SQLite3 both do database-level locking only.

=back

Perl versions 5.8.2 through 5.8.7 are supported.  Prior to 5.8.2, it wasn't
...
OOPS 1.004 has been tested on Linux 2.6.15 (Ubuntu 6.06LTS - Dapper Drake).
OOPS 1.003 was tested on Linux 2.4.23 (Debian unstable as of April '04);
...
lock granularity is the entire database.  Mysql also slows down when
there are multiple simultaneous transactions.  I don't know yet which
back-end is fastest for applications with many simultaneous transactions.
...
plan to scan across large amounts of data because everythign you read
...
record that is locked, not the contents.  Locking C<\%hash> won't another
process from modifying C<$hash-E<gt>{key}>.  Lock C<\$hash-E<gt>{key}>
if you want that.

=back

=head1 LIMITATIONS, ERRATA, DEVELOPMENT STATUS, COMMUNITY, BUGS, AND BUG BOUNTY
...
computer it takes over six hours to run.  I have so much confidence in
my test suite, I'm offering a bounty on bugs!
...
false results, or program failure. Bugs must not be listed here or
be *caused* by using a versions of perl, modules, or RDBMSs that are
...
OOPS caches in memory bit of data that is pulled in during a transaction.
...
emptied when the OOPS instance is destroyed.  Together this means that
...
inaccessible.  I list this here so nobody claims a bounty for breaking
OOPS in this way.
...
I can't think on any good reason to make a circular self-reference, I've
left this hanging.
...
Like perl, OOPS uses reference counting to know when to delete an object.
Unlike a normal program, restarting your program does not clear out the
circular references.
...
For each module, I'll provide a sentence or two that explains why I wrote 
...
using the LDAP backend).  No nesting.  Auto-discovers keys from database
columns.  Has hooks for DBI, LDAP, and custom backends.
...
Must define schema.  
RDBMS schema is created from perl schema.
Can automatically follow references (T2).

=item Rose::DB::Object

Must define schema in package files.  
RDBMS schema is created from perl schema.
Operations are straightforward mappings to SQL.
...
Less transparent.  Requires user to remember a cookie to
retrieve objects.  Cannot handle cross-linking of objects?
No query support?
