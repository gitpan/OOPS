...
#
BEGIN {
	package OOPS::OOPS1004::SelfFilter;
	sub filter
	{
		my $more = filter_read();
...
tie my %qtype, 'OOPS::OOPS1004::debug', sub { return reftype($_[0]) };
tie my %qref, 'OOPS::OOPS1004::debug', sub { return ref($_[0]) };
tie my %qaddr, 'OOPS::OOPS1004::debug', sub { return refaddr($_[0]) };
tie my %qnone, 'OOPS::OOPS1004::debug', sub { $_[0] };
tie my %qmakeref, 'OOPS::OOPS1004::debug', sub { \$_[0] };
tie my %qval, 'OOPS::OOPS1004::debug', sub { return defined $_[0] ? (ref($_[0]) ? "$_[0] \@ $qaddr{$_[0]}" : "'$_[0]'") : 'undef' };
tie my %qplusminus, 'OOPS::OOPS1004::debug', sub { $_[0] >= 0 ? "+$_[0]" : $_[0] };
tie my %caller, 'OOPS::OOPS1004::debug', sub { my $lvls = $_[0]+1; my ($p,$f,$l) = caller($lvls); my $s = (caller($lvls+1))[3]; $s =~ s/OOPS::OOPS1004:://; $l = $f eq __FILE__ ? $l : "$f:$l";  return "$s/$l" };
tie my %qmemval, 'OOPS::OOPS1004::debug', sub { my $v = shift; return "*$v" unless ref $v; return "*$v->[0]/$qval{$v->[1]}" };
tie my %qsym, 'OOPS::OOPS1004::debug', sub { return $typesymbol{reftype(shift)} };

sub OOPS::OOPS1004::debug::TIEHASH { my $p = shift; return bless shift, $p } 
sub OOPS::OOPS1004::debug::FETCH { my $f = shift; return &$f(shift) } 
...
sub new
{
	...
	my ($dbh, $dbms, $prefix) = OOPS::OOPS1004->dbiconnect(%args);
...
END
	...
	return OOPS::OOPS1004::FrontEnd->new($oops);
}
...
{
	my ($oops, $objectid) = @_;
	confess unless $oops->isa('OOPS::OOPS1004');  # XX why?
			...
			while(($id, $pkey, $pval, $ptype) = $objectgrouploadQ->fetchrow_array) {
				...
				if ($t eq 'H') {
					...
					if ($ptype eq '0') {
						...
						#
						$cache->{$id}[$pkey] = undef;
						print "TIE *${id}[$pkey] OOPS::OOPS1004::ObjectInArray object=*$pval\n" if $debug_tie;
						tie $cache->{$id}[$pkey], 'OOPS::OOPS1004::ObjectInArray', $id, $pkey, $pval, $oops;
						$oops->{oldobject}{$id}{$pkey} = $pval;
						print "OLDOBJECT loadobject *$id/$pkey = *$pval (in array)\n" if $debug_oldobject
					} elsif ($ptype eq 'B') {
						$cache->{$id}[$pkey] = $pval;
						print "TIE *${id}[$pkey] OOPS::OOPS1004::BigInArray cksum=$pval\n" if $debug_tie;
						tie $cache->{$id}[$pkey], 'OOPS::OOPS1004::BigInArray', $id, $pkey, $pval, $oops;
					...
					if ($pkey eq $nopkey) {
						...
						if ($ptype eq 'R') {
							...
							print "TIE \$*$id OOPS::OOPS1004::RefObject object=*$pval\n" if $debug_tie;
							tie ${$cache->{$id}}, 'OOPS::OOPS1004::RefObject', $oops, $id, $pval;
						} elsif ($ptype eq 'B') {
							print "\$*$id = '$pval...' -- RefBig\n" if $debug_refalias && defined($pval);
							print "TIE \$*$id OOPS::OOPS1004::RefBig val='$pval'\n" if $debug_tie;
							tie ${$cache->{$id}}, 'OOPS::OOPS1004::RefBig', $oops, $id, $pval;
						} elsif ($ptype eq '0') {
...
#						print "\$*$id load_object cache = untied reference to *$pkey/'$pval' ($qval{$cache->{$id}})\n" if $debug_refalias || $debug_cache;
						...
						print "TIE \$*$id OOPS::OOPS1004::RefAlias target=*$pkey/'$pval' ($qval{$cache->{$id}})\n" if $debug_tie;
						tie $cache->{$id}, 'OOPS::OOPS1004::RefAlias', $oops, $id, $pkey, $pval;
						$oops->{aliasdest}{$pkey}{$id} = $pval;
					}
				} else {
					confess;
...
confess if $oclass->{$id} eq 'OOPS::OOPS1004';
		if ($type->{$id} eq 'H') {
			...
			my $tied = tie %{$cache->{$id}}, 'OOPS::OOPS1004::NormalHash', $new{$id}, $newptype{$id}, $oops, $id;
			$oops->memory($tied, $id);
...
sub load_virtual_object
{
	...
	print "TIE %$objectid OOPS::OOPS1004::DemandHash\n" if $debug_tie;
	my $tied = tie %$obj, 'OOPS::OOPS1004::DemandHash', $oops, $objectid;
...
sub save
{
	...
	confess unless $oops->isa('OOPS::OOPS1004');
...
sub write_array
{
	...
	for (my $index = 0; $index <= $#$obj; $index++) {
		...
		next unless ($tied = tied $obj->[$index]) && $tied->isa('OOPS::OOPS1004::BigInArray');
	...
	for (my $index = 0; $index <= $end; $index++) {
		...
		my $tied;
		if (($tied = tied $obj->[$index]) && $tied =~ /^OOPS::OOPS1004::Demand/ && ! $tied->changed($index)) {
...
{ #debug
	...
	print STDERR "A$dprefix: ".join(',', $q, @debug_args)."\n" if ($debug_queries & 8) 
		&& (($OOPS::OOPS1004::debug_queries & 1) || $debug_match);
} #debug
...
sub DESTROY
{
	...
	print "OOPS::OOPS1004::DESTROY called\n" if $debug_free_tied;
	my $oops = shift;
...
#print "istied($id) = $tied\n";
		next unless $tied;
		next unless $tied =~ /^OOPS::OOPS1004/;
...
#	-	-	-	-	-	-	-	-	-	-	- 
{
	package OOPS::OOPS1004::ObjectInArray;

	use Scalar::Util qw(weaken);
	use Carp qw(longmess);
	our (@ISA) = ('OOPS::OOPS1004::InArray');
...
#	-	-	-	-	-	-	-	-	-	-	- 
{
	package OOPS::OOPS1004::BigInArray;
...
#	-	-	-	-	-	-	-	-	-	-	- 
{
	package OOPS::OOPS1004::RefAlias;
...
#	-	-	-	-	-	-	-	-	-	-	- 
{
	package OOPS::OOPS1004::RefObject;

	our (@ISA) = qw(OOPS::OOPS1004::Ref);
...
#	-	-	-	-	-	-	-	-	-	-	- 
{
	package OOPS::OOPS1004::NormalHash;
	...
	sub DESTROY
	{
		local($main::SIG{'__DIE__'}) = \&OOPS::OOPS1004::die_from_destroy;
		my $self = shift;
		my ($values, $ptypes, $added, $oops, $id, $vars) = @$self;
		delete $tiedvars{$self} if $debug_tiedvars;
	...
	#	tie %{$cache->{$id}}, 'OOPS::OOPS1004::NormalHash', $new{$id}, $newptype{$id}, $oops, $id;
...
#	-	-	-	-	-	-	-	-	-	-	- 
{
	package OOPS::OOPS1004::DemandHash;
...
{
	package OOPS::OOPS1004::FrontEnd;
	...
	{
		my ($pkg, $oops) = @_;
		tie my %x, 'OOPS::OOPS1004::NamedObjects', $oops;
		my $self = bless \%x, $pkg;
...
{
	package OOPS::OOPS1004::NamedObjects;
